...
async function showTournament(ctx: Ctx, edit: boolean): Promise<void> { const data = await readTournament(ctx); const tournament = activeTournament(data); if (!tournament) { const text = "Турнир ещё не составлен — таблица появится после решения организатора."; if (edit) await editTableMessage(ctx, text); else await ctx.reply(text); return; } const entered = tournament.teamIds.map((id) => data.teams[id]).filter((team): team is Team => Boolean(team)); const text = `Подготовка турнира\nПодробный вид\n\n${entered.map((team) => rosterCard(data, team)).join("\n\n")}`; if (edit) await editTableMessage(ctx, text, inlineKeyboard([[inlineButton("Таблица матчей", "matches:show")], [inlineButton("В меню", "menu:main")]])); else await ctx.reply(text); }
